...
**/
(function(){
	function extend(defaultOption,options){
		var temp={};
		for(var i in defaultOption){s
	...
	var C3ListView = {
		builder: function(_container, _adapter, scrollParent){
			...
			var topHeight=0, viewHeight=0, totalHeight=0;
				...
				if(item.view==null){
					...
					if(isAfter){
						...
						if(targetElment==null)
							...
							_container.insertBefore(item.view, targetElment);
					}
					setViewHeight(viewHeight+item.height);
				}
				if(isAfter==false){
